...
function fiveTimes() { //W09 Times Table using Repetition
  //INPUT
  let output = "";
  let result;
  //PROCESSING
...
function loop1() { //W08 Fix Loop Structures
  let output1 = "";
  let i = 0;
  while (i < 2) { //should be smaller than 2
      output1 += `Part 1:${i}<br>`;
      i++;
  }
...
function loop2() { //W08 Fix Loop Structures
  var output2 = "";
  for (let j = 0; j < 3; j++) {
    if (j != 2) { //Only one equal sign
      output2 += `Part 2:${j}, `;
    }
    else {
      output2 += `Part 2:${j}`;
...
function loop3() { //W08 Fix Loop Structures
  let output3 = "";
  for (let k = 0; k < 4; k++) { //Semicolons, not commas
    output3 += k + " ";
  }
  document.getElementById('display3').textContent = output3;
}

function loop4() { //W08 Fix Loop Structures
  let balance = 1000;
  const rate = .10;
  let n = 30;
  for (let s = 1; s < n; s++){ //changed S to start on 1
...
function sumOdds() { //W08 Sum up the Odd Values
...
function retailDiscount() { //W07 Sales Conditional Discount
  //INPUT
...
function weatherShoes() { //W06 Weather Shoes
...
function checkSimpleCalc() { //W06 Simple Calc
...
function cylinderVolume() { //EXAM 1 - Pt. 2
...
function convertPoundsToKilograms() { //W05 Convert Pounds to Kilograms
...
function addNumbers() { //W04 Add Two Numbers Program
  //INPUT
  let x = parseFloat(document.getElementById("number1").value);
  let y = parseFloat(document.getElementById("number2").value);
  //PROCESSING
